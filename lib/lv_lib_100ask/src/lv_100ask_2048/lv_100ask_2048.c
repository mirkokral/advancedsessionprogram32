...
static void btnm_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_DRAW_TASK_ADDED) {
        ...
        if((dsc->id1 >= 0) && (draw_task->type == LV_DRAW_TASK_TYPE_LABEL))
        {
            lv_draw_label_dsc_t * label_dsc = draw_task->draw_dsc;
            uint16_t x, y, num;

            x = (uint16_t)((dsc->id1) / 4);
            y = (dsc->id1) % 4;
            num = (uint16_t)(1 << (game_2048->matrix[x][y]));


            if (num < 8)
                label_dsc->color = LV_100ASK_2048_TEXT_BLACK_COLOR;
            else
                label_dsc->color = LV_100ASK_2048_TEXT_WHITE_COLOR;

        }
        else if((dsc->id1 >= 0) && (draw_task->type == LV_DRAW_TASK_TYPE_FILL)) {
            ...
            rect_dsc->radius = 3;
            rect_dsc->bg_color = get_num_color(num);
        ...
        else if((dsc->id1 == 0) && draw_task->type == LV_DRAW_TASK_TYPE_FILL)
        {
            ...
            rect_dsc->radius = 5;
            rect_dsc->bg_color = LV_100ASK_2048_BG_COLOR;
