...
lv_obj_t *MenuApp::create_dropdown(lv_obj_t *parent, const char *icon, const char *txt, char *choices[], int current)
{
    ...
    lv_obj_t *sw = lv_dropdown_create(obj);
    for (int i = 0; i < sizeof(choices) / sizeof(choices[0]); ++i)
    {
        lv_dropdown_add_option(sw, choices[i], LV_DROPDOWN_POS_LAST);
    }
...
char* tb_choices[] = {"None", "Arrow", "Cursor", "Navigate"};

void trackballModeHandler(lv_event_t *e)
{
    uSettings.trackball_mode = lv_dropdown_get_selected(lv_event_get_target_obj(e));
    settings::write_to_littlefs();
}
...
lv_obj_t *MenuApp::prepareScreen()
{
    ...
    lv_obj_t *sub_tb_page = lv_menu_page_create(menu, NULL);
    lv_obj_set_style_pad_hor(sub_tb_page, lv_obj_get_style_pad_left(lv_menu_get_main_header(menu), 0), 0);
    lv_menu_separator_create(sub_tb_page);
    Serial.println("TB Section");
    section = lv_menu_section_create(sub_tb_page);
    int sel = 0;
    
    cont = create_dropdown(section, LV_SYMBOL_SETTINGS, "Trackball mode", tb_choices, uSettings.trackball_mode);
    lv_obj_add_event_cb(cont, trackballModeHandler, LV_EVENT_VALUE_CHANGED, NULL);
    ...
    Serial.println("Trackball section button");
    cont = create_text(section, LV_SYMBOL_SETTINGS, "Trackball", LV_MENU_ITEM_BUILDER_VARIANT_1);
    lv_menu_set_load_page_event(menu, cont, sub_tb_page);
